...
function wasRequestedFromPopup () {
...
export default function BAPDeprecationModal (props: Props) {
  ...
  const [showModal, setShowModal] = React.useState(
    wasRequestedFromPopup() ||
    shouldShowBAPAlert(onlyAnonWallet, balance.total))

  // After rendering, conditionally open the BAP deprecation popup and listen
  // for the user clicking "Learn more" on that popup.
  React.useEffect(() => {
    if (showModal || !shouldShowBAPPopup(onlyAnonWallet, balance.total)) {
      return () => {}
    }
    ...
    // this page.
    const onHashChange = () => { setShowModal(wasRequestedFromPopup()) }
